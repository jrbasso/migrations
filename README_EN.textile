...
Besides these methods, each migration can have its own callbacks, as in: @beforeCreateTable@, @afterCreateTable@, @beforeDropTable@, ... Besices these callbacks, you can also implement @beforeInstall@, @afterInstall@, @beforeUninstall@ e @afterUninstall@. In any of these callbacks, you can return false if there is an interruption in the migration that is in progress.
